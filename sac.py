...
class Buffer:
    def __init__(self, s_dim, a_dim, cts):
        self.b, self.p, self.bsize = [T.zeros(conf.buffer_limit, i) for i in [s_dim, a_dim if cts else 1, 1, s_dim, 1]], 0, 0  # state, action, reward, s prime, done
...
class PolicyNet(nn.Module):
    def __init__(self, s_dim, a_dim, cts):
        super().__init__()
        self.cts, self.bb = cts, nn.Sequential(nn.Linear(s_dim, 256), nn.ReLU(), nn.Linear(256, 256), nn.ReLU())
        if cts: self.mu, self.log_std = nn.Linear(256, a_dim), nn.Sequential(nn.Linear(256, a_dim))
    ...
    def train_net(self, q1, q2, mb):  # mb: mini-batch
        ...
        q_val = T.min(q1(mb.s, a), q2(mb.s, a)) if self.cts else T.min(q1(mb.s), q2(mb.s)).detach()
        loss = -(q_val - alpha * log_p) if self.cts else -(prob * (q_val - alpha * log_p)).sum(-1)
        optim_step(self.optimizer, loss)
        [q.requires_grad_(True) for q in [q1, q2]]
        alpha_loss = -(self.log_alpha * ((log_p if self.cts else (prob * log_p).sum(-1)) + conf.target_entropy).detach())
        optim_step(self.alpha_optimizer, alpha_loss)

class QNet(nn.Module):
    def __init__(self, s_dim, a_dim, cts):
        super().__init__()
        self.cts, in_dim, out_dim = cts, s_dim + (a_dim if cts else 0), 1 if cts else a_dim
    ...
    def forward(self, s, a=None): return self.net(T.cat([s, a], dim=1) if self.cts else s)

    def train_net(self, tgt, mb):  # mb: mini-batch
        optim_step(self.optimizer, F.mse_loss(self(mb.s, mb.a) if self.cts else self(mb.s).gather(1, mb.a.long()), tgt))

    def soft_update(self, tgt):
        for pt, p in zip(tgt.parameters(), self.parameters()): pt.data.copy_(pt.data * (1 - conf.tau) + p.data * conf.tau)

def calc_target(pi, q1_t, q2_t, mb, cts):  # mb: mini-batch
    with T.no_grad():
        ap, log_p, prob = pi(mb.sp)
        v = (T.min(q1_t(mb.sp, ap), q2_t(mb.sp, ap)) if cts else T.min(q1_t(mb.sp), q2_t(mb.sp))) - pi.log_alpha.exp() * log_p
        return mb.r + conf.gamma * mb.done * (v if cts else (prob * v).sum(-1, keepdim=True))

if __name__ == '__main__':
    envs, cts, s_dim, a_dim = make_envs(conf.env_name, conf.n_envs)
    buf, pi, s, score, n_epi, print_interval = Buffer(s_dim, a_dim, cts), PolicyNet(s_dim, a_dim, cts), envs.reset()[0], 0.0, 0, 20
    q1, q2, q1_t, q2_t = [QNet(s_dim, a_dim, cts) for _ in range(4)]
    ...
    for n_step in tqdm(range(conf.max_timesteps // conf.n_envs), unit_scale=conf.n_envs, unit="step"):
        ...
        if (n_step + 1) * conf.n_envs > conf.learning_starts:
            target = calc_target(pi, q1_t, q2_t, mb := buf.sample(conf.batch_size), cts)
